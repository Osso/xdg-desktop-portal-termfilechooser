...
- `PKGBUILD` — local `arch` sandbox build/test of the exact clean checkout using Cargo offline, package installation, and an `xdg-desktop-portal>=1.17.1` dependency for modern policy selection without deprecated `UseIn` fallback.
